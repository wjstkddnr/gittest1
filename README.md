...
## ✅ 요구사항 명세서
  ...
  ####  :ok: 대화 목록을 저장하고 다시 불러와 사용할 수 있게 하는 것.
...
##### 🎩 ACCUSE_KIM : Django에 비해 FAST-API가 개발 속도가 빠르다는 것을 알았다. DJango보다 FAST-API를 먼저 공부해서 Django도 쉬울 줄 알았지만 생각보다 어렵다는 걸 알았다.
##### 🎩 SUP_WOOK : DB를 활용하여 실시간 모델이 답할 수 있는 데이터를 늘리는 것이 재밌었다. 다음번에는 API를 통해 좀 더 안정적으로 데이터를 업데이트 하는 방안으로 갈 것 같다.
##### 🎩 JAMMANBO_LEE : 선택한 모델에 정보를 넣지 않았을 때 답변이 제대로 나오지 않았지만 RAG를 통해서 답변에 대한 정확도를 높일 수 있었다.
